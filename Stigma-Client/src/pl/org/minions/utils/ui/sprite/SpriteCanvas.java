...
public class SpriteCanvas
{
    ...
    public CanvasLayerGroup newLayerGroupBefore(CanvasLayerGroup group)
    {
        if (layerGroups.contains(group))
            return new CanvasLayerGroup(layerGroups.indexOf(group));
        else
            return new CanvasLayerGroup();
    }
    ...
    public final void render(Graphics2D g)
    {
        ...
        final AffineTransform viewMatrix =
                AffineTransform.getTranslateInstance(-viewportBounds.x,
                                                     -viewportBounds.y);
        ...
        //        g.drawRect(viewportBounds.x, viewportBounds.y,
        //                   viewportBounds.width - 1, viewportBounds.height - 1);

        for (CanvasLayerGroup layerGroup : layerGroups)
            for (CanvasLayer layer : layerGroup.layers)
                layer.render(g);
