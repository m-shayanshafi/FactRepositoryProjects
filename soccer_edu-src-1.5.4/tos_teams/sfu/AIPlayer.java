...
public class AIPlayer extends Thread 
{
	...
	private int			numOfPassDirections = 12;	// is used for passing
	...
	{   
		switch( world.getActionType() )
		{
			...
			case WorldModel.SHOOT: 	 	shootGoal(); 		break;
	...
	private boolean shouldIdribbleFast( WorldModel world )
	{
		...
		world.setKickForce( K_FORCE_MODERATE );
	...
	private boolean shouldIPass( WorldModel world, int numOfDir )
	{
		...
		for( int i=0; i<numOfDir; i++ )
		{
			...
			their_value = getDirectionValueForTeam( opponents, pass_dir );
			
			if( their_value > 1500.0 )
			{
				...
				our_value = getDirectionValueForTeam( teammates, pass_dir );				
				risk = risk + our_value; 
	...
	{
		double dir = aWorldData.getMyself().position
									.direction( aWorldModel.getOppGoal() );
	...
	//
	private double getDirectionValueForTeam( Vector players, double dir )
	{
		int best_time 		= 200, 
		    mean_time 		= 0;
		int player_count 	= 0;
