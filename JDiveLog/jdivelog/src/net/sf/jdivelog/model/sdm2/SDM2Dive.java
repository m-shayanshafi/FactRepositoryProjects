...
public class SDM2Dive implements Comparable<SDM2Dive> {
    ...
    private static final Logger LOGGER = Logger.getLogger(SDM2Dive.class.getName());
    ...
    public JDive asJDive(Masterdata masterdata) {
        ...
        try {
            ...
            LOGGER.log(Level.WARNING, "error parsing date when converting to JDive", e);
        }
        DecimalFormat f = new DecimalFormat("###########,######");
        try {
            d.setDepth(f.parse(depth).doubleValue());
        } catch (ParseException pex) {
            throw new NumberFormatException("pattern: " + f.toPattern() + ", value: " + depth);
        }
        try {
            d.setAverageDepth(f.parse(avgDepth).doubleValue());
        } catch (ParseException pex) {
            throw new NumberFormatException("pattern: " + f.toPattern() + ", value: " + avgDepth);
        }
        ...
        if (partner != null && !"".equals(partner) && divemaster != null && !"".equals(divemaster)) {
            b.append(", ");
        }
        ...
        try {
            ...
            throw new NumberFormatException("pattern: " + f.toPattern() + ", value: " + temperature);
        }
        Tank tank = new Tank();
        if (o2pct != null && !"".equals(o2pct) && !"0".equals(o2pct)) {
            tank.setGas(new Gas());
            try {
                tank.getGas().setTankvolume(f.parse(tankSize).doubleValue()/100000000);
            } catch (ParseException pex) {
                throw new NumberFormatException("pattern: " + f.toPattern() + ", value: " + tankSize);
            }
            ...
            if (pressureStart != null && !"".equals(pressureStart)) {
                tank.getGas().setPstart(Double.parseDouble(pressureStart)*100.0);
            }
        ...
        if (samples.size() > 0) {
            ...
            while (it.hasNext()) {
                ...
                try {
                    ...
                    throw new NumberFormatException("pattern: " + f.toPattern() + ", value: " + s.getDepth());
                }
            }
    ...
    public static String getDiveSitePrivateId(Masterdata masterdata, String spot, String city) {
