...
import mvplan.gui.text.ProfilePrinter;
import mvplan.gui.text.TablePrinter;
...
import net.sf.jdivelog.gui.MixCellEditor;
...
public class MainFrame extends JFrame {
     ...
     * currentTable cannot exist together.
     */
    Profile currentProfile = null; // Current Profile (Single Dive) model

    Model currentModel = null; // Current tissue model

    TableGeneratorModel currentTable = null; // Current multi-profile table

    // model.

    ArrayList<Gas> knownGases; // Maintains known gases

    GasModel knownGasModel; // For Gas table

    ArrayList<SegmentAbstract> knownSegments; // Maintains known Dive Segments
    ...
    private ProgressIndicator progress = new ProgressIndicator(12, 15, null);

    private PageFormat pageFormat; // Remember page setup

    private MainWindow mainWindow;

    public MainFrame(boolean integrated, MainWindow mainWindow) {
        mainFrame = (this);
        ...
        Iterator<Gas> i = Mvplan.prefs.getPrefGases().iterator();
        while (i.hasNext()) {
            knownGases.add(i.next());
        }
        // Copy known segments from Preferences object
        Iterator<SegmentAbstract> si = Mvplan.prefs.getPrefSegments().iterator();
        while (si.hasNext()) {
            knownSegments.add(si.next());
        }
        // Create GUI
        guiInit();
        ...
        if (Mvplan.DEBUG > 0) {
            ...
            if (Mvplan.prefs.getUnits() == Prefs.METRIC)
        ...
        new ConditionsDisplay(textArea);
    }

    /******************* GUI INITIALISATION ********************/

    private void guiInit() {
        if (!integrated && mainWindow == null) {
            mainContentPane = (JPanel) this.getContentPane();
        } else {
            mainContentPane = mainWindow.getMvplanPanel();
        }
        ...
        mainContentPane.add(topPanel, new GridBagConstraints(0, 0, 1, 1, 1.0, 0.0, GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0,
                0), 0, 0));
        mainContentPane.add(mainSplitPane, new GridBagConstraints(0, 1, 1, 1, 1.0, 1.0, GridBagConstraints.SOUTH, GridBagConstraints.BOTH, new Insets(0, 0, 0,
                0), 0, 0));
    ...
    public JPanel getMvPlanContentPane() {
    ...
    private void layoutRightPanel() throws MissingResourceException {
        ...
        rightPanel.add(rightScrollPane, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(10, 10, 10,
                10), 0, 0));
    ...
    private void layoutGasPanel() throws MissingResourceException {
        ...
        TableColumn gascol = gasTable.getColumnModel().getColumn(0);
        gascol.setMinWidth(150);
        gascol.setWidth(150);
        gascol.setPreferredWidth(150);
        MixCellEditor gce = new MixCellEditor(mainWindow, mainWindow.getGasDatabase());
        gascol.setCellEditor(gce);
        gascol.setCellRenderer(gce);
        ...
        gasPanel.add(gasScrollPane, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.NORTH, GridBagConstraints.BOTH, new Insets(10, 10, 5, 10),
                0, 0));
        ...
        gasPanel.add(gasToolPanel, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.SOUTHWEST, GridBagConstraints.HORIZONTAL, new Insets(0, 0,
                0, 0), 0, 0));
    ...
    private void layoutDivePanel() throws MissingResourceException {
        ...
        divePanel.add(diveScrollPane, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.NORTH, GridBagConstraints.BOTH,
        ...
        divePanel.add(diveToolPanel, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.SOUTH, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0,
                0), 0, 0));
        ...
        // In order to get better behavior for the JTable, configure custom cell
        // editors
    ...
    private void layoutRepetitivePanel() throws MissingResourceException {
        ...
        repetitiveDivePanel.add(repetitiveModeCB, new GridBagConstraints(0, 0, 1, 1, 0.1, 1.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(5,
                0, 5, 5), 0, 0));
        repetitiveDivePanel.add(repetitiveLabel, new GridBagConstraints(1, 0, 1, 1, 0.3, 1.0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(5,
                0, 5, 5), 0, 0));
        repetitiveDivePanel.add(surfaceIntervalField, new GridBagConstraints(2, 0, 1, 1, 0.3, 1.0, GridBagConstraints.WEST, GridBagConstraints.NONE,
                new Insets(5, 5, 5, 5), 0, 0));
        repetitiveDivePanel.add(tissueComponent, new GridBagConstraints(3, 0, 1, 1, 0.3, 1.0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0,
                0, 0, 0), 0, 0));
    }
    ...
    private void layoutLeftPanel() throws MissingResourceException {
        ...
        leftPanel.add(repetitiveDivePanel, new GridBagConstraints(0, 0, 1, 1, 1.0, 0.0, GridBagConstraints.NORTH, GridBagConstraints.BOTH, new Insets(5, 5, 5,
                5), 0, 0));
        leftPanel
                .add(divePanel, new GridBagConstraints(0, 1, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0));
        leftPanel.add(gasPanel, new GridBagConstraints(0, 2, 1, 1, 1.0, 1.0, GridBagConstraints.NORTH, GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0));
    }
    ...
    private void layoutToolbar() throws MissingResourceException {
        ...
        progress.setPreferredSize(new Dimension(25, 25));
        ...
        topProgressPanel.add(progress, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0),
                0, 0));
        topPanel.add(topProgressPanel, new GridBagConstraints(1, 0, 1, 1, 1.0, 1.0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 5, 0, 5),
                0, 0));
        ...
        prefsButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                // If extendedLimits or Units changed need to update table
                // renderer limits
                // So do it anyway
    ...
    private void saveModel() {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (lastModelFile.exists()
                    && (JOptionPane.showConfirmDialog(this, Mvplan.getResource("mvplan.gui.MainFrame.saveModelOverwriteDialog.text"), Mvplan
                            .getResource("mvplan.gui.MainFrame.saveModelOverwriteDialog.title"), JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE) != JOptionPane.OK_OPTION)) {
                // Do nothing
                ...
                if (dao.saveModel(currentModel, lastModelFile.toString()) != ModelDAO.SUCCESS)
                    JOptionPane.showMessageDialog(this, Mvplan.getResource("mvplan.gui.MainFrame.saveModelErrorDialog.text"), Mvplan
                            .getResource("mvplan.gui.MainFrame.saveModelErrorDialog.title"), JOptionPane.ERROR_MESSAGE);
            }
        }
    }
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (currentModel != null) {
                ...
                if (currentModel.getUnits() != Mvplan.prefs.getUnits()) {
                    JOptionPane.showMessageDialog(this, Mvplan.getResource("mvplan.gui.MainFrame.loadModelUnitErrorDialog.text"), Mvplan
                            .getResource("mvplan.gui.MainFrame.loadModelUnitErrorDialog.title"), JOptionPane.ERROR_MESSAGE);
                    clearDive();
                } else {
                ...
                JOptionPane.showMessageDialog(this, Mvplan.getResource("mvplan.gui.MainFrame.loadModelErrorDialog.text"), Mvplan
                        .getResource("mvplan.gui.MainFrame.loadModelErrorDialog.title"), JOptionPane.ERROR_MESSAGE);
                clearDive();
            }
        }
    }
    ...
    private void updateGasCombo() {
        ...
        Iterator<Gas> it = knownGases.iterator();
        while (it.hasNext()) {
            gasComboBox.addItem(it.next());
        }
    }
    ...
    private void printProfileDisplayComponent() {
        ...
        Iterator<SegmentAbstract> i = knownSegments.iterator();
        SegmentAbstract header_segment = i.next();
        double depth_header = header_segment.getDepth();
        String gas_header = header_segment.getGas().toString();
        // Make heading like this:
        // "MV-Plan Beta 3    GF:20/90 1000m"
        String heading = String.format("%1$s m  %2$s", depth_header, gas_header);
        if (gasOCDecoCB.isSelected())
            heading = "Bailout:  " + heading;
        if (Mvplan.prefs.getAltitude() > 0.0)
            heading = heading + String.format("  %1$4.0f%2$s", Mvplan.prefs.getAltitude(), Mvplan.prefs.getDepthShortString());
        if (currentModel != null) {
            if (currentProfile.getIsRepetitiveDive())
                heading = heading + "   SI:" + currentProfile.getSurfaceInterval();
            new TablePreviewDialog(this, currentProfile.getProfile(), heading);
        } else if (currentTable != null)
            new TablePreviewDialog(this, currentTable, heading);
    ...
    public void savePrefs() {
        ...
        Mvplan.prefs.setPrefGases(new ArrayList<Gas>()); // Zap old object
        Iterator<Gas> i = knownGases.iterator();
        while (i.hasNext()) {
            Mvplan.prefs.getPrefGases().add(i.next());
        }
        // Update prefs with known Segments
        Mvplan.prefs.setPrefSegments(new ArrayList<SegmentAbstract>());
        Iterator<SegmentAbstract> si = knownSegments.iterator();
        while (si.hasNext()) {
            Mvplan.prefs.getPrefSegments().add(si.next());
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog((this), Mvplan.getResource("mvplan.gui.MainFrame.prefsSaveErrorDialog.info.text") + '\n' + Mvplan.prefFile, Mvplan
                    .getResource("mvplan.gui.MainFrame.prefsSaveErrorDialog.title.text"), JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void doTableGenerator(JTextArea text) {
        int[] modifiers = Mvplan.prefs.getModifiers(); // holds array of time
        // modifiers
        int returnCode;
        ...
        // Create new multiDiveDialog and initialise it with the multiProfile
        // model
        mdd = new TableGeneratorDialog(this, mp);
        // Show dialog. Returns with boolean true to continue. Modifiers may
        // have been altered.
        if (mdd.showDialog()) {
            ...
            case Profile.CEILING_VIOLATION:
                JOptionPane.showMessageDialog((this), Mvplan.getResource("mvplan.gui.MainFrame.ceilingViolation.text"), Mvplan
                        .getResource("mvplan.gui.MainFrame.ceilingViolation.title"), JOptionPane.ERROR_MESSAGE);
                break;

            case Profile.NOTHING_TO_PROCESS:
                JOptionPane.showMessageDialog((this), Mvplan.getResource("mvplan.gui.MainFrame.noSegments.text"), Mvplan
                        .getResource("mvplan.gui.MainFrame.noSegments.title"), JOptionPane.INFORMATION_MESSAGE);
                break;
            case Profile.PROCESSING_ERROR:
                JOptionPane.showMessageDialog((this), Mvplan.getResource("mvplan.gui.MainFrame.processingError.text"), Mvplan
                        .getResource("mvplan.gui.MainFrame.processingError.title"), JOptionPane.ERROR_MESSAGE);
            case Profile.INFINITE_DECO:
                JOptionPane.showMessageDialog((this), Mvplan.getResource("mvplan.gui.MainFrame.decoNotPossible.text"), Mvplan
                        .getResource("mvplan.gui.MainFrame.decoNotPossible.title"), JOptionPane.ERROR_MESSAGE);
            default:
                break;
            }
        }
    ...
    private void doDive() {
        ...
        if (!repetitiveMode || currentModel == null) {
            ...
            if (p.isDiveSegments()
                    || JOptionPane.showConfirmDialog(this, Mvplan.getResource("mvplan.gui.MainFrame.noSegmentsDoSurfaceInterval.text"), Mvplan
                            .getResource("mvplan.gui.MainFrame.noSegmentsDoSurfaceInterval.title"), JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) { // Is
                // there
                // anything
                // to
                // process
                p.doSurfaceInterval(Integer.parseInt(surfaceIntervalField.getText()));
                textArea.append(Mvplan.getResource("mvplan.gui.MainFrame.modelUpdated.text") + " " + p.getSurfaceInterval()
        ...
        returnCode = p.doDive();
        if ((Mvplan.DEBUG > 0))
            System.out.println("doDive: return code=" + returnCode);
        switch (returnCode) {
        case Profile.SUCCESS:
            ...
            saveButton.setEnabled(true);
            setTissueIcon();
            new ProfilePrinter(currentProfile, textArea).doPrintTable();
            break;
    ...
    /**
     * setTissueIcon(): Sets tissue status icon and tooltip depending on the
     * state of the model
     */
    ...
    private void doConvertDepths(int oldUnits) {
        Iterator<Gas> i1 = knownGases.iterator();
        Iterator<SegmentAbstract> i2 = knownSegments.iterator();
        // Set conversion factor depending on direction of conversion
        double conversion = (oldUnits == Prefs.METRIC ? Prefs.METERS_TO_FEET : 1 / Prefs.METERS_TO_FEET);
        // Go through gases
        while (i1.hasNext()) {
            Gas g = i1.next();
    ...
    private void doAbout() {
        ...
        s = s + "<p>\u00A9 2005-2008 Guy Wittig&nbsp</p>";
    ...
    private class DivePopupListener extends MouseAdapter {
        ...
        private void doPopup(MouseEvent e) {
            // This caters for the PC (right button) and Mac (CTRL + left
            // button)
            if ((e.getButton() == MouseEvent.BUTTON3) || ((e.getModifiers() & InputEvent.CTRL_MASK) != 0 && e.getButton() == MouseEvent.BUTTON1)) {
    ...
    /**
     * Used to listen to focus events on table cells and pre-select the whole
     * field
     */
