...
public class ProfilePrinter {
    ...
    public ProfilePrinter(Profile p, JTextArea text) {
        this.profile=p;
        this.textArea=text; 
    ...
    private void doPrintExtendedTable() {        
        ...
        textArea.append(Mvplan.getResource("mvplan.gui.text.ProfilePrinter.settings.text")+"="+
                        (int)(Mvplan.prefs.getGfLow()*100.)+"-"+(int)(Mvplan.prefs.getGfHigh()*100.));
        ...
        Iterator<SegmentAbstract> i = segments.iterator();
        while(i.hasNext()) {
            s=i.next();
    ...
    private void doGasUsage() {
        ...
        // Display gas usage        
        ArrayList<Gas> gases=profile.getGases();
        Iterator<Gas> i2 = gases.iterator();  
