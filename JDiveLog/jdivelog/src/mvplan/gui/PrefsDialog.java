...
public class PrefsDialog extends JDialog implements ActionListener, FocusListener
{
	...
	private static final long serialVersionUID = 1652302084499115007L;
    ...
    public PrefsDialog(Frame frame)
    {
        ...
        currentUnits = Prefs.METRIC;     // Metric unless otherwise advised 
        ...
        stopPanel.setLayout(new GridLayout(3,2));
        ...
        divePanel.setLayout(new GridLayout(3,2));  
        ...
        optionsPanel.setPreferredSize(new Dimension(150,110));
        ...
        if(Mvplan.prefs.getUnits()==Prefs.METRIC) {
            ...
            currentUnits = Prefs.METRIC;            
        } else { 
            imperialButton.setSelected(true);  
            currentUnits = Prefs.IMPERIAL;
        }
        ...
        Action escapeAction = new AbstractAction(){
			private static final long serialVersionUID = 2006421990599004281L;
        ...
        Action limitsAction = new AbstractAction() {
			private static final long serialVersionUID = -4717761471657898607L;
        ...
        if ("ok".equals(e.getActionCommand())) {            
            ...
            try {
                ...
                if(metricButton.isSelected())
                    Mvplan.prefs.setUnitsTo(Prefs.METRIC);
                else    
            ...
            if(currentUnits != Prefs.METRIC) {
                // Convert units to metric
                // Field validation to  metric
                stopDepthMax = stopDepthMax / Prefs.METERS_TO_FEET;
                stopDepthMin = stopDepthMin / Prefs.METERS_TO_FEET;
                ascentRateMax = ascentRateMax / Prefs.METERS_TO_FEET;
                ascentRateMin = ascentRateMin / Prefs.METERS_TO_FEET; 
                descentRateMax = descentRateMax / Prefs.METERS_TO_FEET;
                descentRateMin = descentRateMin / Prefs.METERS_TO_FEET;   
                try {
                    stopDepthIncrement.setText(String.valueOf(  (int)roundDouble(0,  ( Double.parseDouble(stopDepthIncrement.getText()) / Prefs.METERS_TO_FEET+0.01)) )) ;
                    lastStopDepth.setText(String.valueOf(  roundDouble( 0, Double.parseDouble(lastStopDepth.getText()) / Prefs.METERS_TO_FEET) )) ; 
                    descentRate.setText(String.valueOf(  roundDouble( 0,  Double.parseDouble(descentRate.getText()) / Prefs.METERS_TO_FEET) )) ;
                    ascentRate.setText(String.valueOf(  roundDouble( 0,  Double.parseDouble(ascentRate.getText()) / Prefs.METERS_TO_FEET) )) ;
                    ...
                    altitude.setText(String.valueOf(  roundDouble( 0,  Double.parseDouble(altitude.getText()) / Prefs.METERS_TO_FEET) )) ; 
                } catch (NumberFormatException ex) {
                    // No action
                }
                currentUnits = Prefs.METRIC;
            ...
            if(currentUnits != Prefs.IMPERIAL) {
    ...
    class AltitudeVerifier extends InputVerifier {
        ...
        {
            JTextField field = (JTextField)comp;
            double altitudeMax= Prefs.ALTITUDE_MAX;            
            boolean passed=false;
            
            // Modify maximum for imperial if required
            if(currentUnits==Prefs.IMPERIAL) altitudeMax=altitudeMax*Prefs.METERS_TO_FEET;
