...
public class TableGeneratorDialog extends JDialog implements FocusListener {
	private static final long serialVersionUID = 9130304386003800387L;
    ...
    public TableGeneratorDialog(Frame parentFrame, TableGeneratorModel multiProfile) {        
        ...
        Action escapeAction = new AbstractAction(){
			private static final long serialVersionUID = -4378764856694936015L;
    ...
    public boolean showDialog() {
        ...
        SegmentAbstract s;
        //String str;
        ArrayList<SegmentAbstract> knownSegments, modifiableSegments=new ArrayList<SegmentAbstract>();
        
        if (cs>=0) {            
            // New multi segment code
            knownSegments = mp.getKnownSegments();           
            Iterator<SegmentAbstract> it=knownSegments.iterator();
            while(it.hasNext()){
                s = it.next();
                if(s.getTime()>0.0)
