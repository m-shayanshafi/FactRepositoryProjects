...
{
	private static final long serialVersionUID = 2068804266270931219L;
     ...
     */    
    public Compartment() {  
        // Initialize compartment pressures to zero
        ppHe=0;
        ppN2=0;               
    }
    ...
    public void constDepth(double ppHeInspired, double ppN2Inspired, double segTime) throws ModelStateException
    {
       ...
       if(ppHeInspired<0.0 || ppN2Inspired <0.0 || segTime <0.0){
        ...
        ppHeNew = ppHe + ((ppHeInspired-ppHe) * (1-Math.exp(-kHe*segTime)));
        ppN2New = ppN2 + ((ppN2Inspired-ppN2) * (1-Math.exp(-kN2*segTime)));
     ...
     * @param ppHeInspired Partiap pressure of inspired helium
     * @param ppN2Inspired Partial pressure of inspired Nitrogen
     * @param rateHe Rate of chnage of ppHe
     * @param rateN2 Rate of chnage of ppHe
     * @param segTime Segment time in minutes
     * @throws mvplan.model.ModelStateException Throws ModelStateException if partial pressures are < 0.0 or time < 0.0
     */
    ...
    {       
        ...
        if(ppHeInspired<0.0 || ppN2Inspired<0.0 || segTime < 0.0) {
            ...
            ppHeNew = ppHeInspired + rateHe * (segTime - (1.0/kHe)) - (ppHeInspired - ppHe - (rateHe/kHe)) * Math.exp(-kHe*segTime);
            ppN2New = ppN2Inspired + rateN2 * (segTime - (1.0/kN2)) - (ppN2Inspired - ppN2 - (rateN2/kN2)) * Math.exp(-kN2*segTime);        
            ppHe = ppHeNew;                         // Update compartment
            ppN2 = ppN2New;        
    	}
    }
