...
public class Mvplan
{
    ...
    public static final String BUILD_DATE = "7-Jun-2008";     // Application release date
    private static final float MIN_JVM = 1.5f;               // Minimum Java JVM requirement. Checked on startup.
    static final String PREF_FILE = new File(System.getProperty("user.home"), ".jdivelog-mvplan.xml").getAbsolutePath(); // Preferences file name
    ...
    public static Locale preferredLocale=Messages.getLocale();     // For language preference
    ...
    public Mvplan(boolean integrated, MainWindow mainWindow)
    {     
        ...
        if(vmVersion < MIN_JVM) {
            // Display error
            new mvplan.gui.VersionFrame(String.valueOf(vmVersion),String.valueOf(MIN_JVM));                
        } else {     
            ...
            // Open main screen             
            frame = new MainFrame(integrated, mainWindow);
    ...
    public MainFrame getmvplanFrame() {
        return frame;
    }    
