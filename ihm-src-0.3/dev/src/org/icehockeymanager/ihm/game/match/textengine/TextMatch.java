...
public class TextMatch extends Match {
  ...
  public synchronized void moveOn() {
    ...
    // Period not started ? Let's get it on !! && Tell the listeners
    ...
    // Play until break (Scene end, period end, or sudden death, or any other
    // break from the plays)
    boolean breaker = false;
    while (!breaker) {

      breaker = isBreak();

      // If game's on ... rock n' roll !
      if (!breaker) {

        // Not in play ? Play face off !
    ...
    // Now update everything and tell the listener that the scene has
    // finished in the best and only way.
    ...
    default: {
      // In the end, check if the score has changed or jsut finish the scene
  ...
  private int checkTimeAndStatus(boolean updateStats) {
    ...
    if (matchSnapshot.currentState == MatchSnapshot.MATCH_STATE_REGULAR) {
      ...
      if (matchSnapshot.currentSeconds >= matchSnapshot.currentNextBreak) {
        // Regular game finished and no overtime needed (anyway)
      ...
      // Overtime Period finished ?
      if (matchSnapshot.currentSeconds >= matchSnapshot.currentNextBreak) {
        // contignous overtime or game finished ...
  ...
  public void goalScored() {
    MatchDataGoal goal = new MatchDataGoal(matchSnapshot.currentSeconds, matchSnapshot.currentPuckHoldingBlock.getTeam(), matchSnapshot.getOpponentsBlock().getTeam(), matchSnapshot.currentPuckHolder, matchSnapshot.currentPuckChain[0],
        matchSnapshot.currentPuckChain[1]);
    ...
    // Check the game score (for overtime & shootout)
