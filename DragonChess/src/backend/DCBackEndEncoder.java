/*
 * Classname			: DCBackEndEncoder
 * Author				: TheBlackUnicorn <TheBlackUnicorn@softhome.net>
 * Creation Date		: Sat Oct 19 14:11:45 CEST 2002 
 * Last Updated			: Fri Dec 06 16:16:24 CET 2002 
 * Description			: Encodes DCMessages
 * GPL disclaimer		:
...
public class DCBackEndEncoder {
	 ...
	 * @return	DCMessage to send
	 */ 
	public DCMessage newPlayerRegistered(String name, String address,
			int playerNumber) {
		//wrap int in Integers
		...
		DCMessage message = new DCMessage(
				DCConstants.MSG_NEW_PLAYER_REGISTERED,
				DCConstants.TO_FRONTEND,
				DCConstants.PLAYER_BOTH,
				true,
				data);
	...
	public DCMessage playerUnregistered(int player, String reason) {
		...
		DCMessage message = new DCMessage(
				DCConstants.MSG_PLAYER_UNREGISTERED,
				DCConstants.TO_FRONTEND,
				DCConstants.PLAYER_BOTH,
				true,
				data);
	 ...
	 * his attempt has succeeded. {@link connectivity.DCGame} uses this message to link 
	 ...
	 * @return	DCMessage to send
	 */
	public DCMessage registerSuccess(String name, String address,
			int playerNumber, int connectionNumber) {
		//wrap int in Integers
		Integer playerNumberInteger = new Integer(playerNumber);
		...
		data[2] = playerNumberInteger;
		DCMessage message = new DCMessage(
				DCConstants.MSG_REGISTER_SUCCESS,
				DCConstants.TO_FRONTEND,
				playerNumber,
				false,
				data);
	...
	public DCMessage registerFailure(int connectionNumber) {
		DCMessage message = new DCMessage(
				DCConstants.MSG_REGISTER_FAILURE,
				DCConstants.TO_FRONTEND,
				DCConstants.PLAYER_NONE, //receiver is NOT a player
				false);
		message.setConnection(connectionNumber);
		return message;
	}
	 ...
	 * @return	DCMessage to send
	 */
	public DCMessage setPlayerInfo(int player, String name, String address,
		...
		data[2] = address;
		DCMessage message = new DCMessage(
				DCConstants.MSG_SET_PLAYER_INFO,
				DCConstants.TO_FRONTEND,
	...
	public DCMessage pieceSelected(int player, DCCoord location) {
		DCMessage message = new DCMessage(
				DCConstants.MSG_PIECE_SELECTED,
				DCConstants.TO_FRONTEND,
				player,
				false,
				location);
	...
	public DCMessage pieceNotSelected(int player, DCCoord location, int reason) {
		...
		DCMessage message = new DCMessage(
				DCConstants.MSG_PIECE_NOT_SELECTED,
				DCConstants.TO_FRONTEND,
				player,
				false,
				data);
	...
	public DCMessage pieceDeselected(int player, DCCoord location, DCMoveList list) {
		...
		data[1] = list;
		DCMessage message = new DCMessage(
				DCConstants.MSG_PIECE_DESELECTED,
				DCConstants.TO_FRONTEND,
				player,
				false,
				data);
	...
	public DCMessage pieceNotMoved(int player, DCCoord location, int reason) {
		Object []data = {location, new Integer(reason)};
		DCMessage message = new DCMessage(
				DCConstants.MSG_PIECE_NOT_MOVED,
				DCConstants.TO_FRONTEND,
				player,
				false,
				data);
	...
	public DCMessage moveNotUndone(int reason, int player) {
		DCMessage message = new DCMessage(
				DCConstants.MSG_MOVE_NOT_UNDONE,
				DCConstants.TO_FRONTEND,
				player,
				false,
				new Integer(reason));
	...
	public DCMessage piecePromoted(DCExtCoord promotion) {
		DCMessage message = new DCMessage(
				DCConstants.MSG_PIECE_PROMOTED,
				DCConstants.TO_FRONTEND,
				DCConstants.PLAYER_BOTH,
				true,
				promotion);
	...
	public DCMessage loadDCFrontEndDump(int player, DCFrontEndDump dump) {
		DCMessage message = new DCMessage(
				DCConstants.MSG_LOAD_DUMP,
				DCConstants.TO_FRONTEND,
				player,
				//if to all players, also to spectators
				(player == DCConstants.PLAYER_BOTH) ? true : false,
				dump);
		return message;
	}
	...
	public DCMessage removeHistoryString(int player) {
		DCMessage message = new DCMessage(
				DCConstants.MSG_REMOVE_HISTORY,
				DCConstants.TO_FRONTEND,
				DCConstants.PLAYER_BOTH,
				true,
				new Integer(player));
	...
	public DCMessage pieceRestored(DCExtCoord coord) {
		DCMessage message = new DCMessage(
				DCConstants.MSG_PIECE_RESTORED,
				DCConstants.TO_FRONTEND,
				DCConstants.PLAYER_BOTH,
				true,
				coord);
	 ...
	 * @return 	DCMessage to send
	 */
	public DCMessage chatMessage(boolean publicBoolean, 
			String playerName, String msg) {
		Object []data = { new Boolean(publicBoolean),
						  playerName,
						  msg };
		DCMessage message = new DCMessage(
				DCConstants.MSG_PLAYER_CHAT_RECV,
				DCConstants.TO_FRONTEND,
				DCConstants.PLAYER_BOTH,
				publicBoolean,
				data);
