  ...
  extends Control
{
  ...
  @Override void draw(IVideoDriver driver, Point2i offset)
  {
    ...
    if (image != null)
    {
      ...
      {
        if (value > i)
          rectSrc.set(1, 1, imageWidth / 2 - 1, imageHeight - 1);
        else
          rectSrc.set(imageWidth / 2 + 1, 1, imageWidth - 1, imageHeight - 1);
        rectDest.set(r.x1 + i * r.getWidth() / 10 , r.y1,
          r.x1 + (i + 1) * r.getWidth() / 10, r.y2);
        driver.draw2DImage(image, rectDest, rectSrc, null, color, true);
      }
