...
public class GUIScene
{
  ...
  public GUIScene(String id, ISceneManager sceneManager, IVideoDriver videoDriver)
  {
    ...
    try
    {
      ...
      Document doc = reader.read("data/scenes/" + id + ".xml");
      Element root = doc.getRootElement();
      //id
      this.id = XMLReader.readAttributeValue(root, "id");
      //weather
      Element eWeather = root.element("weather");
      if (eWeather != null)
      {
      ...
      //stadium
      Element eStadium = root.element("stadium");
      if (eStadium != null)
      {
        ...
        if (stadiumID.length() > 0)
        {
          ...
          if (XMLReader.readAttributeValueBoolean(eStadium, "light") &&
            weather != null && weather.isStadiumLighting())
          {
      ...
      //program
      Element eProgram = root.element("program");
      if (eProgram != null)
      {
      ...
      //nativeCamera.setFOV(0.256636f);
      nativeCamera.setFOV(0.015f);
      videoDriver.setAmbientLight(new SColorf(1, 1, 1, 1));
      camera = new Camera1(sceneManager, nativeCamera);
      //nativeCamera.setPosition(new vector3df(0, 1000, -2000));
      //load camera flights
      Element eFlights = root.element("cameraflights");
      if (eFlights != null)
      {
        List eFlightList = eFlights.elements("cameraflight");
        for (int iFlight = 0; iFlight < eFlightList.size(); iFlight++)
        {
          Element eFlight = (Element) eFlightList.get(iFlight);
          ...
          Element eFrom = eFlight.element("from");
          if (eFrom != null)
          {
            List ePointsList = eFrom.elements("point");
            for (int i = 0; i < ePointsList.size(); i++)
            {
              Element ePoint = (Element) ePointsList.get(i);
          ...
          Element eAt = eFlight.element("at");
          if (eAt != null)
          {
            List ePointsList = eAt.elements("point");
            for (int i = 0; i < ePointsList.size(); i++)
            {
              Element ePoint = (Element) ePointsList.get(i);
              Vector3fFlags point = readPoint(ePoint);
   ...
   */
  private Vector3fFlags readPoint(Element ePoint)
  {
