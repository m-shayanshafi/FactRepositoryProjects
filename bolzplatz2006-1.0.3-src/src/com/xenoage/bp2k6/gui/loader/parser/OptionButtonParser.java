...
public class OptionButtonParser
{
  ...
  public static OptionButton parseOptionButton(Element e,
    Screen parentScreen, Group parentGroup)
  {
    ...
    //options
    List listOptionContainers = e.elements("options");
    if (listOptionContainers.size() > 0)
    {
      Element optionsContainer = (Element) listOptionContainers.get(0);
      List listOptions = optionsContainer.elements("option");
      String systemList = optionsContainer.attributeValue("systemlist");
      ...
      if (listOptions.size() > 0)
      {
        ...
        for (int i = 0; i < listOptions.size(); i++)
        {
          ...
          optionsCaption.add(el.getText());
          String val = el.attributeValue("value");
          if (val == null) val = el.getText(); //if no value is set, use caption
