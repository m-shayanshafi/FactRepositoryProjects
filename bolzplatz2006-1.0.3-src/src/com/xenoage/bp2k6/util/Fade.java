...
public class Fade
{
  ...
  public Fade(IVideoDriver videoDriver)
  {
    ...
    if (ScreenHeight >= 1024) TextureSize =1024;
    if (videoDriver.queryFeature(E_VIDEO_DRIVER_FEATURE.EVDF_RENDER_TO_TARGET))
    {
      try
      {
        rt = videoDriver.createRenderTargetTexture(new dimension2di(TextureSize, TextureSize));
      }
      catch (Exception Ex)
      {
        rt = null;
  ...
  public static void doFade2(IVideoDriver videoDriver)
  {
    ...
    float gettime;
    int d = 32;
    float w = ((float)TextureSize * d) / ScreenWidth, h = ((float)TextureSize * d) / ScreenHeight;
    ...
    else
    {
      if (rttime != 0 && !fillrt2)
      {
        ...
        else
        {
          ...
          {
            case 0:
              rtart = (new java.util.Random()).nextInt(2) + 1; //ANDI: only small image and falling stones
            case 102:
              videoDriver.draw2DRectangle(
                new SColor(255 -
                           (int) (255 * (gettime - rtstarttime) /
                                  (rttime - rtstarttime)), 0, 0, 0), rectScreen);
              break;
            case 101:
              videoDriver.draw2DRectangle(
                new SColor(255 -
                           (int) (255 * (gettime - rtstarttime) /
                                  (rttime - rtstarttime)), 255, 255, 255), rectScreen);
              break;
//Black
            case 10:
              videoDriver.draw2DRectangle(
                new SColor(128, 0, 0, 0), rectScreen);
              rtart = 102;
              break;
...
//White
            case 9:
              videoDriver.draw2DRectangle(
                new SColor(128, 255, 255, 255), rectScreen);
              rtart = 101;
              break;
...
//Einfaches Überblenden, buggy wegen fehlerhafter draw2DImage Implementation (diese schweine)
            case 8:
              videoDriver.draw2DImage(rt, rectScreen, rectTexture,
                null, new SColor(255 -
                  (int) (255 * (gettime - rtstarttime) / (rttime - rtstarttime)),
                255, 255, 255)
                );
              break;
              //Linien wegziehen
            ...
            case 1:
              ...
              //kleiner
              a = (int) (ScreenWidth * 0.5 * (gettime - rtstarttime) /
                         (rttime - rtstarttime));

              //Zuweisung
              p1.setX( (int) (ScreenHeight * 0.25 * (gettime - rtstarttime) /
                              (rttime - rtstarttime)) + a);
              //Newton:
              //s=1/2*a*t^2+b*t
              p1.setY( (int) ( (2 *
                                (200 + ScreenHeight) /
                                ( (rttime - rtstarttime) *
                                 (rttime - rtstarttime))) * 0.5 *
                              (gettime - rtstarttime) *
                              (gettime - rtstarttime) -
                              200 * (gettime - rtstarttime) /
                              (rttime - rtstarttime) + 0.75*a));
              rect1.setUpperLeftCorner(p1);
              ...
              for (int x = 0; x < ScreenWidth / d; x++)
              {
                for (int y = ScreenHeight / d; y >= 0; y--)
                {
                  ...
                  a = (int) ( (2 *
                               ...
                               ( (rttime - rtstarttime) *
                                (rttime - rtstarttime))) * 0.5 *
                             (gettime - rtstarttime) * (gettime - rtstarttime));
