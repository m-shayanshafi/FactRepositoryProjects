...
public class Resolution
{
   ...
   */
  public int getSelectedVideoMode() throws DocumentException, Exception
  {
    SAXReader reader = new SAXReader();
    reader.setEncoding(XMLReader.encoding);
    Document document = reader.read("data/config/video.xml");
  ...
  public void saveSelectedVideoMode(int res, boolean fullscreen) throws IOException
  {
    ...
    Element root = document.addElement( "resolution" );
    if (res > -1)
    {
      root.addElement("width").addText(""+reso[res][0]);
      root.addElement("height").addText(""+reso[res][1]);
      root.addElement("depth").addText(""+reso[res][2]);
    }
    else
    {
      root.addElement("width").addText("640");
      root.addElement("height").addText("480");
      root.addElement("depth").addText("16");
    }
    root.addElement("fullscreen").addText(""+fullscreen);

    File out = new File("data/config/video.xml");
    ...
    XMLWriter writer = new XMLWriter(
      new FileWriter("data/config/video.xml"), outf);
...
//    writer.write( document );
  ...
  public boolean getSelectedFullscreenMode() throws Exception
  {
    ...
    return Boolean.parseBoolean(
      document.getRootElement().element("fullscreen").getText());
  }
