...
public class Stadium
{
  ...
  public Stadium(String id)
  {
    ...
    try
    {
      ...
      if (new java.io.File("data/config/newstadium/" + id + ".xml").exists())
        doc = reader.read("data/config/newstadium/" + id + ".xml");
      else if (new java.io.File("data/temp/" + id + ".xml").exists())
        doc = reader.read("data/temp/" + id + ".xml");
      else
        doc = reader.read("data/stadiums/" + id + ".xml");
      Element root = doc.getRootElement();
      
      //terrain
      Element eTerrain = root.element("terrain");
      if (eTerrain != null)
      {
      ...
      //field and lawn
      int lawnQuality = 0;
      Element eLawn = root.element("lawn");
      if (eLawn != null)
      {
      ...
      //career stand blocks
      Element eCareerStandBlocks = root.element("careerstandblocks");
      if (eCareerStandBlocks != null)
      {
        List listBlocks = eCareerStandBlocks.elements("block");
        for (int i = 0; i < listBlocks.size(); i++)
        {
          Element eBlock = (Element) listBlocks.get(i);
      ...
      Element eAdboardStandBlocks = root.element("careeradboardblocks");
      if (eAdboardStandBlocks != null)
      {
        List listBlocks = eAdboardStandBlocks.elements("block");
        for (int i = 0; i < listBlocks.size(); i++)
        {
          Element eBlock = (Element) listBlocks.get(i);
          int alignment = XMLReader.readAttributeValueInt(eBlock, "alignment") - 1;
      ...
      //stands
      Element eStands = root.element("stands");
      if (eStands != null)
      {
        List listStands = eStands.elements("stand");
        int i = 0;
        stadiumInfo.setMaxSpectators(0);
        for (i = 0; i < listStands.size(); i++)
        {
          Element eStand = (Element) listStands.get(i);
      ...
      //adboards
      Element eAdboards = root.element("adboards");
      if (eAdboards != null)
      {
        List listAdboards = eAdboards.elements("adboard");
        int i = 0;
        for (i = 0; i < listAdboards.size(); i++)
        {
          Element eAdboard = (Element) listAdboards.get(i);
      ...
      //scoreboard
      Element eScoreboard = root.element("scoreboard");
      if (eScoreboard != null)
      {
      ...
      //floodlights
      Element eFloodlights = root.element("floodlights");
      if (eFloodlights != null)
      {
        List listFloodlights = eFloodlights.elements("floodlight");
        int i = 0;
        for (i = 0; i < listFloodlights.size(); i++)
        {
          Element eFloodlight = (Element) listFloodlights.get(i);
      ...
      //attractions
      Element eAttractions = root.element("attractions");
      if (eAttractions != null)
      {
        //hot dog stand
        Element eHotDogStand = eAttractions.element("hotdogstand");
        ...
        //betting office
        Element eBettingOffice = eAttractions.element("bettingoffice");
        ...
        //beer bar
        Element eBeerBar = eAttractions.element("beerbar");
        if (eBeerBar != null)
        {
          beerBar = new BeerBar();
        ...
        //track
        Element eTrack = eAttractions.element("track");
        if (eTrack != null)
        {
          track = new Track();
        ...
        //high jump
        Element eHighJump = eAttractions.element("highjump");
        if (eHighJump != null)
        {
      ...
      //objects
      Element eObjects = root.element("objects");
      if (eObjects != null)
      {
        List listObjects = eObjects.elements("object");
        int i = 0;
        for (i = 0; i < listObjects.size(); i++)
        {
          Element eObject = (Element) listObjects.get(i);
    ...
    throws Exception
  {
    ...
    //root element
    Element root = document.addElement("stadium");
    root.addAttribute("id", id);
    root.addAttribute("name", stadiumInfo.getName());
    
    //terrain
    if (terrain != null)
    {
      Element eTerrain = root.addElement("terrain");
      eTerrain.addAttribute("id", terrain.getID());
    }
    
    //lawn
    Element eLawn = root.addElement("lawn");
    if (field != null)
      eLawn.addAttribute("quality", String.valueOf(field.getLawnQuality()));
    else
      eLawn.addAttribute("quality", "12");
    
    //goals
    Element eGoals = root.addElement("goals");
    String goalstype;
    ...
    eGoals.addAttribute("id", goalstype);
    if (stadiumInfo.isWoodenGoalsBought())
      eGoals.addAttribute("wooden", "true");
    if (stadiumInfo.isAluminumGoalsBought())
      eGoals.addAttribute("aluminum", "true");
    
    //career stand blocks
    Element eCareerStandBlocks = root.addElement("careerstandblocks");
    for (int iStage = 0; iStage < 3; iStage++)
    {
      for (int iAlignment = 0; iAlignment < 8; iAlignment++)
      {
        ...
        {
          Element eBlock = eCareerStandBlocks.addElement("block");
          eBlock.addAttribute("stage", String.valueOf(iStage + 1));
          eBlock.addAttribute("alignment", String.valueOf(iAlignment + 1));
        }
      }
    }
    
    //career adboard blocks
    Element eCareerAdboardBlocks = root.addElement("careeradboardblocks");
    for (int iAlignment = 0; iAlignment < 8; iAlignment++)
    {
      if (builtCareerAdboardBlocks[iAlignment])
      {
        Element eBlock = eCareerAdboardBlocks.addElement("block");
        eBlock.addAttribute("alignment", String.valueOf(iAlignment + 1));
      }
    }
    
    //stands
    Element eStands = root.addElement("stands");
    eStands.addAttribute("maxspectators", String.valueOf(
      stadiumInfo.getMaxSpectators()));
    for (int i = 0; i < stands.size(); i++)
    {
      Stand stand = stands.get(i);
      Element eStand = eStands.addElement("stand");
      eStand.addAttribute("id", stand.getID());
      eStand.addAttribute("modification", stand.getModification());
      ...
      if (stage > 0 && alignment > 0)
      {
        eStand.addAttribute("stage", String.valueOf(stage));
        eStand.addAttribute("alignment", String.valueOf(alignment));
      }
    }
    
    //adboards
    Element eAdboards = root.addElement("adboards");
    for (int i = 0; i < adboards.size(); i++)
    {
      Adboard adboard = adboards.get(i);
      Element eAdboard = eAdboards.addElement("adboard");
      eAdboard.addAttribute("id", adboard.getID());
      adboard.addPositionAndRotationToXMLElement(eAdboard);
      
      int alignment = adboard.getCareerBlockAlignment();
      if (alignment > 0)
      {
        eAdboard.addAttribute("alignment", String.valueOf(alignment));
      }
    }
    ...
    if (scoreboard != null)
    {
      Element eScoreboard = root.addElement("scoreboard");
      eScoreboard.addAttribute("stage", String.valueOf(scoreboard.getStage()));
      scoreboard.addPositionAndRotationToXMLElement(eScoreboard);
    }
    
    //floodlights
    Element eFloodlights = root.addElement("floodlights");
    for (int i = 0; i < floodlights.size(); i++)
    {
      Floodlight floodlight = floodlights.get(i);
      Element eFloodlight = eFloodlights.addElement("floodlight");
      eFloodlight.addAttribute("stage", String.valueOf(floodlight.getStage()));
      if (floodlight.getCareerPosition() != 0)
        eFloodlight.addAttribute("careerposition",
          String.valueOf(floodlight.getCareerPosition()));
      floodlight.addPositionAndRotationToXMLElement(eFloodlight);
    }
    
    //attractions
    Element eAttractions = root.addElement("attractions");
    if (hotDogStand != null)
    {
      Element eHotDogStand = eAttractions.addElement("hotdogstand");
      hotDogStand.addPositionAndRotationToXMLElement(eHotDogStand);
    }
    if (bettingOffice != null)
    {
      Element eBettingOffice = eAttractions.addElement("bettingoffice");
      bettingOffice.addPositionAndRotationToXMLElement(eBettingOffice);
    }
    if (beerBar != null)
    {
      Element eBeerBar = eAttractions.addElement("beerbar");
      beerBar.addPositionAndRotationToXMLElement(eBeerBar);
    }
    if (track != null)
    {
      Element eTrack = eAttractions.addElement("track");
      track.addPositionAndRotationToXMLElement(eTrack);
    }
    if (highJump != null)
    {
      Element eHighJump = eAttractions.addElement("highjump");
      highJump.addPositionAndRotationToXMLElement(eHighJump);
    }
    
    //objects
    Element eObjects = root.addElement("objects");
    for (int i = 0; i < objects.size(); i++)
    {
      SimpleStadiumObject object = objects.get(i);
      Element eObject = eObjects.addElement("object");
      eObject.addAttribute("id", object.getID());
      object.addPositionAndRotationToXMLElement(eObject);
    }

    File out;
    if (!tempFolder)
      out = new File("data/stadiums/" + id + ".xml");
    else
      out = new File("data/temp/" + id + ".xml");
   ...
   * "G�hnende Langeweile", "Erlebnispark".
   */
