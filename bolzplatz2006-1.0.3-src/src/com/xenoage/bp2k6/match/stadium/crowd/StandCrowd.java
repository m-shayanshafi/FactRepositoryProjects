...
public class StandCrowd
{
    ...
    boolean celebration)
  {
    ...
    try
    {
      ...
      Document doc = reader.read("data/stands/" + stand.getID() + ".xml");
      Element root = doc.getRootElement();
      //modification
      Element eModificationList = root.element("modifications");
      if (eModificationList == null)
        throw new Exception("No modifications list found!");
      List eModifications = eModificationList.elements("modification");
      for (int iMod = 0; iMod < eModifications.size(); iMod++)
      {
        Element eMod = (Element) eModifications.get(iMod);
        if (XMLReader.readAttributeValue(eMod, "id").equals(stand.getModification()))
        {
          //load all available characters
          Element eCrowd = eMod.element("crowd");
          if (eCrowd != null)
          {
            List listChars = eCrowd.elements("char");
            ...
            {
              Element eChar = (Element) listChars.get(iChar);
              int type;
          ...
          Element eBengalLight = eMod.element("bengallight");
