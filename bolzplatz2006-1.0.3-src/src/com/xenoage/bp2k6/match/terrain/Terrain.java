...
public class Terrain
{
  ...
  public Terrain(String id, ISceneManager sceneManager, IVideoDriver videoDriver)
  {
    ...
    try
    {
      ...
      Document doc = reader.read("data/terrains/" + id + ".xml");
      ...
      //load textures
      Element textures = root.element("textures");
      if (textures != null)
      {
        List listTextures = textures.elements("texture");
        for (int i = 0; i < listTextures.size(); i++)
        {
          Element t = (Element) listTextures.get(i);
      ...
      Element ePeriphery = root.element("periphery");
      if (ePeriphery != null)
      {
        List listMeshes = ePeriphery.elements("mesh");
        for (int i = 0; i < listMeshes.size(); i++)
        {
          Element t = (Element) listMeshes.get(i);
          String src = XMLReader.readAttributeValue(t, "src");
