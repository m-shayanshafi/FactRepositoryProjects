...
public class WorldCup
{
  ...
  public WorldCup(int id) throws TeamNotFoundException
  {
    ...
    try
    {
      ...
      Document doc = reader.read("data/savegames/worldcup/worldcup" + id + ".xml");
      ...
      if (eGroups != null)
      {
        List listGroups = eGroups.elements("league");
        ...
        {
          groups[i] = new League((Element) listGroups.get(i));
        }
      }
      else
      {
      ...
      //load finals
      Element eFinals = root.element("finals");
      if (eFinals != null)
      {
        List listMatches = eFinals.elements("match");
        for (int i = 0; i < listMatches.size(); i++)
        {
          finalMatches[i] = new MatchInfo((Element) listMatches.get(i));
        }
      }
  ...
  public static ArrayList<String> getTeamIDs(boolean realTeams)
  {
    ...
    try
    {
      ...
      List listListTeams = root.elements("teams");
      for (int iListTeams = 0; iListTeams < listListTeams.size(); iListTeams++)
      {
        Element eTeams = (Element) listListTeams.get(iListTeams);
        if ((!realTeams && 
        ...
        {
          //load list of teams
          List listTeams = eTeams.elements("team");
          for (int iTeams = 0; iTeams < listTeams.size(); iTeams++)
          {
            Element eTeam = (Element) listTeams.get(iTeams);
  ...
  public void saveToXMLFile(int id)
  {
    ...
    try
    {
      ...
      root.addAttribute("gamemode", "worldcup");
      //user team id
      root.addAttribute("userteamid", userTeamID);
      //current matchday
      root.addAttribute("matchdaynumber", String.valueOf(matchDayNumber));
      ...
      //save the 8 groups
      Element eGroups = root.addElement("groups");
      for (int i = 0; i < 8; i++)
      {
        groups[i].saveToXML(eGroups);
      }
      //save the final matches, if already played
      Element eFinals = root.addElement("finals");
      for (int i = 0; i < 16; i++)
      {
      ...
      File out = new File("data/savegames/worldcup/worldcup" + id + ".xml");
