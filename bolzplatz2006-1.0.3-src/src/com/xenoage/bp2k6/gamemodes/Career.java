...
public class Career
{
  ...
  public Career(int id) throws TeamNotFoundException
  {
    ...
    try
    {
      ...
      Document doc = reader.read("data/savegames/career/career" + id + ".xml");
      ...
      //list of seasons
      Element eSeasons = root.element("seasons");
      if (eSeasons != null)
      {
        currentSeason = new Season(eSeasons.element("season"));
      }
      else
      {
    ...
    ArrayList<String> international)
  {
    ...
    try
    {
      ...
      Element root = doc.getRootElement();
      //leagues
      Element eLeagues = root.element("leagues");
      if (eLeagues != null)
      {
        List listLeagues = eLeagues.elements("league");
        for (int iLeague = 0; iLeague < listLeagues.size(); iLeague++)
        {
          Element eLeague = (Element) listLeagues.get(iLeague);
          int type = XMLReader.readAttributeValueInt(eLeague, "type");
          List listTeams = eLeague.elements("team");
          for (int iTeam = 0; iTeam < listTeams.size(); iTeam++)
          {
            Element eTeam = (Element) listTeams.get(iTeam);
  ...
  public void saveToXMLFile(int id)
  {
    ...
    try
    {
      ...
      //root element
      Element root = document.addElement("savegame");
      root.addAttribute("id", String.valueOf(id));
      root.addAttribute("gamemode", "career");
      //user team id
      root.addAttribute("userteamid", userTeamID);
      //the user's money
      root.addAttribute("usermoney", String.valueOf(userMoney));
      //current season (year)
      root.addAttribute("currentyear", String.valueOf(currentYear));
      //current league
      root.addAttribute("currentleague", String.valueOf(currentLeague));
      //current season matchday (needed for fast loading of career information)
      root.addAttribute("currentmatchday",
        String.valueOf(currentSeason.getCurrentMatchDay()));
      //current league matchday (needed for fast loading of career information)
      root.addAttribute("currentleaguematchday",
      ...
      root.addAttribute("currentallstarsleaguematchday",
        String.valueOf(getCurrentAllstarsMatchDayNumber()));
      //list of seasons
      Element eSeasons = root.addElement("seasons");
      currentSeason.saveToXML(eSeasons);
      File out = new File("data/savegames/career/career" + id + ".xml");
