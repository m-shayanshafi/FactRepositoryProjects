...
public class MatchDay
{
   ...
   */
  public MatchDay(Element eMatchDay)
  {
    ...
    //list of all matches
    Element eMatches = eMatchDay.element("matches");
    if (eMatches != null)
    {
      List listMatches = eMatches.elements("match");
      for (int i = 0; i < listMatches.size(); i++)
      {
        MatchInfo match = new MatchInfo((Element) listMatches.get(i));
        matches.add(match);
      }
    }
    //table after the matchday, if available
    Element eTable = eMatchDay.element("table");
    if (eTable != null)
    {
  ...
  public void saveToXML(Element parentElement)
  {
    ...
    Element eMatchDay = parentElement.addElement("matchday");
    //number of the matchday
    eMatchDay.addAttribute("number", String.valueOf(number));
    //matchday already played?
    eMatchDay.addAttribute("matchdayplayed", String.valueOf(matchDayPlayed));
    //list of all matches
    Element eMatches = eMatchDay.addElement("matches");
    for (int i = 0; i < matches.size(); i++)
    {
      MatchInfo match = matches.get(i);
