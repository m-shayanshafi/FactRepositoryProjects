...
public class AllstarsLeague
{
   ...
   */
  public AllstarsLeague(Element eAllstarsLeague)
  {
    ...
    Element eTeams = eAllstarsLeague.element("teams");
    if (eTeams != null)
    {
    ...
    //load the groups
    Element eGroups = eAllstarsLeague.element("groups");
    if (eGroups != null)
    {
    ...
    Element eFinals = eAllstarsLeague.element("finals");
    if (eFinals != null)
    {
      List listFinals = eFinals.elements("match");
      for (int i = 0; i < listFinals.size(); i++)
      {
        finalMatches[i] = new MatchInfo((Element) listFinals.get(i));
      }
    }
  }
  ...
  public void saveToXML(Element parentElement)
  {
    ...
    Element eAllstarsLeague = parentElement.addElement("allstarsleague");
    //add current matchday
    eAllstarsLeague.addAttribute("currentmatchday",
      String.valueOf(currentMatchDayNumber));
    //list of all teams
    Element eTeams = eAllstarsLeague.addElement("teams");
    for (int i = 0; i < teamIDs.size(); i++)
    {
    ...
    //save the groups
    Element eGroups = eAllstarsLeague.addElement("groups");
    for (int i = 0; i < groupsCount; i++)
    {
      groups[i].saveToXML(eGroups);
    }
    //save the final matches, if already played
    Element eFinals = eAllstarsLeague.addElement("finals");
    for (int i = 0; i < 8; i++)
    {
