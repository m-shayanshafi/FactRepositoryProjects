...
public class MatchInfo
{
   ...
   */
  public MatchInfo(Element eMatchInfo)
  {
    //parse match element
    ...
    //list of all goals
    Element eGoals = eMatchInfo.element("goals");
    if (eGoals != null)
    {
      List listGoals = eGoals.elements("goal");
      for (int i = 0; i < listGoals.size(); i++)
      {
        Goal goal = new Goal((Element) listGoals.get(i));
        goals.add(goal);
      }
    }
  }
  ...
  public void saveToXML(Element parentElement)
  {
    ...
    Element eMatch = parentElement.addElement("match");
    eMatch.addAttribute("played", String.valueOf(played));
    eMatch.addAttribute("home", teamHomeID);
    eMatch.addAttribute("visitor", teamVisitorID);
    eMatch.addAttribute("drawallowed", String.valueOf(drawAllowed));
    //list of all goals
    Element eGoals = eMatch.addElement("goals");
    for (int i = 0; i < goals.size(); i++)
    {
      Goal goal = goals.get(i);
