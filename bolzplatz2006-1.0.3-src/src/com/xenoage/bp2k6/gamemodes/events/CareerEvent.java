...
public class CareerEvent
{
   ...
   */
  public CareerEvent(Element eEvent)
  {
    //read before match action set
    Element eBeforeMatch = eEvent.element("beforematch");
    if (eBeforeMatch != null)
      this.beforeMatchActionSet = new ActionSet(eBeforeMatch);
    //read after match action set
    Element eAfterMatch = eEvent.element("aftermatch");
    if (eAfterMatch != null)
      this.afterMatchActionSet = new ActionSet(eAfterMatch);
    //read title and text
    Element eTitle = eEvent.element("title");
    if (eTitle != null)
      this.title = eTitle.getText();
    Element eText = eEvent.element("text");
    if (eText != null)
    {
