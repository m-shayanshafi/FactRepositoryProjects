...
public class ConditionsParser
{
   ...
   * nothing was found.
   */
  public static ArrayList<Condition> parse(Element eConditions)
  {
    ArrayList<Condition> ret = new ArrayList<Condition>();
    for (int i = 0; i < eConditions.elements().size(); i++)
    {
      Element e = (Element) eConditions.elements().get(i);
      Condition c = null;
      if (e.getName().equals("money"))
        c = new MoneyCondition(e);
      else if (e.getName().equals("league"))
        c = new LeagueCondition(e);
      else if (e.getName().equals("matchresult"))
        c = new MatchResultCondition(e);
      else if (e.getName().equals("lawn"))
