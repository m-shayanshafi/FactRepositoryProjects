...
public abstract class AbstractScoreBoardEventServlet extends AbstractRegisterServlet
{
	...
	protected boolean visitEvents(String key, ScoreBoardEventVisitor visitor) {
		try {
			Queue q = ((QueueingListener)clientMap.get(key)).q;
			ScoreBoardEvent sbE;
			while (null != (sbE = (ScoreBoardEvent)q.poll()))
				sbE.accept(visitor);
		} catch ( Exception e ) {
			return false;
		}
