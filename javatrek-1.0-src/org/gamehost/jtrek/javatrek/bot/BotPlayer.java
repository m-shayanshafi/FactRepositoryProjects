...
import org.gamehost.jtrek.javatrek.Trek3DPoint;
import org.gamehost.jtrek.javatrek.TrekBot;
import org.gamehost.jtrek.javatrek.TrekBuoy;
import org.gamehost.jtrek.javatrek.TrekCoordHistory;
import org.gamehost.jtrek.javatrek.TrekDrone;
import org.gamehost.jtrek.javatrek.TrekMath;
import org.gamehost.jtrek.javatrek.TrekMine;
import org.gamehost.jtrek.javatrek.TrekObject;
import org.gamehost.jtrek.javatrek.TrekObserverDevice;
import org.gamehost.jtrek.javatrek.TrekPlanet;
import org.gamehost.jtrek.javatrek.TrekPlayer;
import org.gamehost.jtrek.javatrek.TrekServer;
import org.gamehost.jtrek.javatrek.TrekShip;
import org.gamehost.jtrek.javatrek.TrekShipDebris;
import org.gamehost.jtrek.javatrek.TrekStarbase;
    ...
    protected void checkEnemies() {
        ...
        if (botPeace) {
            ...
            for (Enumeration e = humanShips.elements(); e.hasMoreElements(); ) {
                ...
                if (curShip.lockTarget == ship || curShip.scanTarget == ship) {
                    humanShipCount++;
    ...
    protected void evaluateMode() {
        if (destinationObj != null || targetShip != null) {
            ...
            if (shipSubMode == ATTACK) {
                if (TrekMath.canScanShip(ship, targetShip) && ship.scanTarget != targetShip) ship.scanTarget = targetShip;
            }
        }
                                ...
                                switch (targetShip.torpedoType) {
                                    ...
                                    case (TrekShip.TORPEDO_BOLTPLASMA):
                                        if (ship.maxTorpedoRange > targetShip.maxTorpedoRange) {
                                            ...
                                            if (ship.maxCruiseWarp == targetShip.maxCruiseWarp && ship.warpEnergy >= (ship.maxWarpEnergy * .85)) doOverwarp = true;
    ...
    protected void sendMessage() {
        if (msgTimeout <= 0 && receivedMsgs.length() > 1 && shipSubMode == NONE && shipMode == PATROL) {
            ...
            if (msgShip != null) {
                ...
                if ( (messageReceived.indexOf("fuck") != -1) || (messageReceived.indexOf("shit") != -1) || (messageReceived.indexOf("asshole") != -1) ) {
                ...
                if (messageReceived.indexOf("lame") != -1 || messageReceived.indexOf("cheat") != -1 || messageReceived.indexOf("stupid") != -1) {
                    int randomMsgVal = Math.abs(gen.nextInt() % 5);
                    switch (randomMsgVal) {
                        case 0:
                ...
                if (messageReceived.equalsIgnoreCase("hi") || messageReceived.indexOf("hi ") != -1 || messageReceived.indexOf("hello") != -1 ||
                ...
                if (messageReceived.indexOf("human") != -1 || messageReceived.indexOf("bot") != -1) {
                    int randomMsgVal = Math.abs(gen.nextInt() % 5);
                    switch (randomMsgVal) {
                        case 0:
                            responseMsg = "huh?\r";
                ...
                if (messageReceived.indexOf("ping") != -1 || messageReceived.indexOf("where") != -1) {
                ...
                if (messageReceived.indexOf("fight") != -1 || messageReceived.indexOf("coward") != -1 ||
