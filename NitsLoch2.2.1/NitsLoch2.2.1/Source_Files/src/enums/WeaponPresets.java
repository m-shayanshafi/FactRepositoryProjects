...
/**
 * Preset configurations for weapon shops.  There are up to 20 types
 * of weapon shops, each one can have different weapons available at
...
public enum WeaponPresets {
	...
	/**
	 * Adds a type of weapon to this preset.
	 * @param itemID int : type of weapon
	 * @param price int : price of weapon
	 */
	public void addWeapon(int itemID, int price){
		if(items.contains(StoreItems.values()[itemID])) return;
		items.add(StoreItems.values()[itemID]);
		prices.add(price);
	}
	
	public void deleteWeapon(int itemID){
		int itemIndex = -1;
