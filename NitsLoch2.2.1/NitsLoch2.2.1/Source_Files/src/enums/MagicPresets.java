...
/**
 * Preset configurations for magic shops.  There are up to 20 types
 * of magic shops, each one can have different kinds of magic available at
 * different prices.  This enum will keep track of those preset shop
 * configurations.
 * @author Darren Watts
 * date 1/12/08
 */
public enum MagicPresets {
	P_000,
	P_001,
	P_002,
	P_003,
	 ...
	 */
	private MagicPresets(){
		items = new ArrayList<StoreItems>();
		prices = new ArrayList<Integer>();
	}
	
	/**
	 * Clears the Magic presets
	 */
	...
	/**
	 * Adds a type of magic to this preset.
	 * @param itemID int : type of magic
	 * @param price int : price of magic
	 */
	public void addMagic(int itemID, int price){
		if(items.contains(StoreItems.values()[itemID])) return;
		items.add(StoreItems.values()[itemID]);
		prices.add(price);
	}
	
	public void deleteMagic(int itemID){
		int itemIndex = -1;
	...
	/**
	 * Gets the list of magic items assigned to this preset.  It assigns
	 * the appropriate amount of money to the items that it's
	 * keeping track of, and then returns a list of the items.
	 * @return ArrayList<StoreItems> : Magic item types.
	 */
