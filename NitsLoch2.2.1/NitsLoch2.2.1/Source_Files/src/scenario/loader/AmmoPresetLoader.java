...
/**
 * This class will be passed the XML element for reading the ammoPresets
 ...
 *
 */
public class AmmoPresetLoader {
	/**
	 * Constructor for the AmmoPresetLoader.  Takes the ammoPresets node in the
	 * scenario file as a parameter.  Throws an exception if the information
	 * in the ammoPresets section of the scenario file isn't formatted
	 * correctly/complete.
	 * @param node Node : ammoPresets node
	 * @throws BadScenarioFileException
	 */
	public AmmoPresetLoader(Node node) throws BadScenarioFileException {
		try {
			processPreset(node);
		...
		try {
			...
			for(Node list : filteredNodeList){
				...
				for(int i = 0; i < items.getLength(); i++){
					...
					AmmoPresets.values()[currentType].addAmmo(itemID, price);
				}
			}
		} catch(Exception e){
			System.err.println("Your ammo preset information is invalid.");
			throw new BadScenarioFileException();
		}
	}
}
