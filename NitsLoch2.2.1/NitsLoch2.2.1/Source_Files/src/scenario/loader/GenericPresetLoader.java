...
/**
 * This class will be passed the XML element for reading the genericPresets
 ...
 *
 */
public class GenericPresetLoader {
	
	/**
	 * Constructor for the genericPresetLoader.  Takes the genericPresets node in the
	 * scenario file as a parameter.  Throws an exception if the information
	 * in the genericPresets section of the scenario file isn't formatted
	 * correctly/complete.
	 * @param node Node : genericPresets node
	 * @throws BadScenarioFileException
	 */
	public GenericPresetLoader(Node node) throws BadScenarioFileException {
		try {
			processPreset(node);
		...
		try {
			...
			for(Node list : filteredNodeList){
				...
				for(int i = 0; i < items.getLength(); i++){
					...
					GenericPresets.values()[currentType].addGenericItem(itemID, price);
				}
			}
		} catch(Exception e){
			System.err.println("Your generic preset information is invalid.");
			throw new BadScenarioFileException();
		}
	}
}
