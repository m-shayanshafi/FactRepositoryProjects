	...
	public void processBar(Node node) throws BadScenarioFileException {
		try {
			...
			for(int i = 0; i < filteredNodeList.size(); i++){
				...
				tonicWater = currentElement.getElementsByTagName(
				"tonicwater").item(0).getFirstChild().getNodeValue();

				soda = currentElement.getElementsByTagName(
				"soda").item(0).getFirstChild().getNodeValue();
				
				gin = currentElement.getElementsByTagName(
				"gin").item(0).getFirstChild().getNodeValue();
				
				rum = currentElement.getElementsByTagName(
				"rum").item(0).getFirstChild().getNodeValue();
				
				scotch = currentElement.getElementsByTagName(
				"scotch").item(0).getFirstChild().getNodeValue();
				
				redeye = currentElement.getElementsByTagName(
				"redeye").item(0).getFirstChild().getNodeValue();
				
				Bars.values()[currentType].setStats(tonicWater, soda, gin,
						rum, scotch, redeye);
