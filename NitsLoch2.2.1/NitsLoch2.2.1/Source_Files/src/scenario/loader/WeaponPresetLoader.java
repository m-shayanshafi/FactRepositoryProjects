...
/**
 * This class will be passed the XML element for reading the weaponPresets
 ...
 *
 */
public class WeaponPresetLoader {
	/**
	 * Constructor for the WeaponPresetLoader.  Takes the weaponPresets node in the
	 * scenario file as a parameter.  Throws an exception if the information
	 * in the weaponPresets section of the scenario file isn't formatted
	 * correctly/complete.
	 * @param node Node : weaponPresets node
	 * @throws BadScenarioFileException
	 */
	public WeaponPresetLoader(Node node) throws BadScenarioFileException {
		try {
			processPreset(node);
		...
		try {
			...
			for(Node list : filteredNodeList){
				...
				for(int i = 0; i < items.getLength(); i++){
					...
					WeaponPresets.values()[currentType].addWeapon(itemID, price);
				}
			}
		} catch(Exception e){
			System.err.println("Your weapon preset information is invalid.");
			throw new BadScenarioFileException();
		}
	}
}
