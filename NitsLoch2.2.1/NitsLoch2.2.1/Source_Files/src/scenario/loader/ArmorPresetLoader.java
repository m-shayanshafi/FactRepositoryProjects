...
/**
 * This class will be passed the XML element for reading the armorPresets
 ...
 *
 */
public class ArmorPresetLoader {
	
	/**
	 * Constructor for the ArmorPresetLoader.  Takes the armorPresets node in the
	 * scenario file as a parameter.  Throws an exception if the information
	 * in the armorPresets section of the scenario file isn't formatted
	 * correctly/complete.
	 * @param node Node : armorPresets node
	 * @throws BadScenarioFileException
	 */
	public ArmorPresetLoader(Node node) throws BadScenarioFileException {
		try {
			processPreset(node);
		...
		try {
			...
			for(Node list : filteredNodeList){
				...
				for(int i = 0; i < items.getLength(); i++){
					...
					ArmorPresets.values()[currentType].addArmor(itemID, price);
				}
			}
		} catch(Exception e){
			System.err.println("Your armor preset information is invalid.");
			throw new BadScenarioFileException();
		}
	}
}
