...
public class HospitalShop extends JFrame {
	...
	private void setPriceLabels(){
		...
		if(player.getMoney() >= neededHealth / src.Constants.HP_PER_DOLLAR){
			...
			lblPriceNum.setText(String.valueOf(neededHealth / 
					Constants.HP_PER_DOLLAR+1));
		}
		else{
			lblPriceNum.setText(String.valueOf(player.getMoney()));
			lblHealthNum.setText(String.valueOf(player.getMoney() * 
					Constants.HP_PER_DOLLAR));
		}
	}
	...
	public void btnMore_actionPerformed(ActionEvent e) {
		...
		if(player.getHitPoints() + Integer.parseInt(lblHealthNum.getText()) >= 
			player.getMaxHitPoints() || 
			Integer.parseInt(lblPriceNum.getText()) == player.getMoney()){ 
			// If already as high as it can get, do nothing.
			return;
		}
		else{ // Can add more
			// 3 or less from max.
			if(Integer.parseInt(lblHealthNum.getText()) + 3 >= player.getMaxHitPoints()){
				...
				lblPriceNum.setText(String.valueOf(Integer.parseInt
						(lblHealthNum.getText()) / Constants.HP_PER_DOLLAR+1));
			}
		}
	}
	...
	public void btnLess_actionPerformed(ActionEvent e) {
		// If it would result in no healing, do nothing.
		if(Integer.parseInt(lblHealthNum.getText()) - 3 <= 0)
			return;
		else{
			lblHealthNum.setText(String.valueOf(Integer.parseInt(lblHealthNum.getText()) - 3));
			lblPriceNum.setText(String.valueOf(Integer.parseInt
	...
	public void btnOK_actionPerformed(ActionEvent e) {
		...
		if(Integer.parseInt(lblHealthNum.getText()) > 0){
			Messages.getInstance().addMessage("You just purchased " +
					Integer.parseInt(lblHealthNum.getText()) + " hit points.");
